	...
	<LineItemBase :lineItem="lineItem">
		<template #data>
			...
			<div class="control">
				<div class="select">
					<select v-model="lineItem.amount" @change="update()" :ref="`grossLossSelect-${id}`">
						<option value="default" disabled>Select Previous Gross Loss Amount</option>
			...
			<div class="control" v-if="creatingGrossLoss === false">
				<div class="select">
					<select v-model="lineItem.newAmount" @change="update()" :ref="`newGrossLossSelect-${id}`">
						<option value="default" disabled>Select New Gross Loss Amount</option>
						<option v-for="estimate in estimates" :value="estimate.gross_loss" v-text="estimate.gross_loss"></option>
						<option v-if="invoice.feeSchedule.erroneous" value="erroneous">erroneous</option>
						<option v-if="invoice.feeSchedule.cwop"value="cwop">CWOP</option>
					</select>
...
<script>
	...
	export default {
		name: 'DifferenceInTiersLineItem',
		...
		methods: {
			...
			async createNewGrossLoss() {
				...
				await this.getGrossLossSelect().then(ele => {
					...
					ele.selectedIndex = 1;
					this.lineItem.newAmount = ele.options[ele.selectedIndex].value
					ele.dispatchEvent(e);
				});
			},
			...
			async getGrossLossSelect(id = 0) {
				const self = this;
				return new Promise((resolve, reject) => {
					self.$nextTick(() => resolve(self.$refs[`newGrossLossSelect-${id}`]))
		 		});
			},
		}
	}
